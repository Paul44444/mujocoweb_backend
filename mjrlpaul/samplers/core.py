import logging
import numpy as np
from mjrlpaul.utils.gym_env import GymEnv
from mjrlpaul.utils import tensor_utils
...
def do_rollout(
    ...
    if type(env) == str:
        env = GymEnv(env)
        print("\n A")
    elif isinstance(env, GymEnv) or isinstance(env, gym.Env):
        env = env
        print("\n B")
        print(f"\n obs: {env.observation_space}")
        #print(f"env.horizon: {env.horizon}")
    elif callable(env):
        env = env(**env_kwargs)
        print("\n C")
    else:
    ...
    print(f"env: {env}, horizon: {horizon}")
    print(f"env: {env}, horizon: {horizon}")
    print(f"env: {env}, horizon: {horizon}")
    print(f"env: {env}, horizon: {horizon}")
    print(f"env: {env}, horizon: {horizon}")
    ...
    for ep in range(num_traj):
        ...
        while t < horizon and done != True:
            ...
            next_o, r, done, _, env_info_step = env.step(a)
            ...
            if isinstance(env_info_base['time'], np.ndarray): #01052026
                env_info_base['time'] = env_info_base['time'][0]
                env_info_base['obs_dict']['time'] = env_info_base['obs_dict']['time'][0]
...
def sample_paths(
    ...
    #results = do_rollout(num_traj, env, policy)
    if True:#01052026
