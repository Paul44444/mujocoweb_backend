...
from mjrlpaul.utils.make_train_plots import make_train_plots
from mjrlpaul.utils.gym_env import GymEnv
from mjrlpaul.samplers.core import sample_paths
import numpy as np
import pickle
...
def train_agent_new(job_name, agent,
                seed = 0,
                niter = 101,
                gamma = 0.995,
                gae_lambda = None,
    ...
    for i in range(i_start, niter):
        ...
        N = num_traj if sample_mode == 'trajectories' else num_samples
        args = dict(N=N, sample_mode=sample_mode, gamma=gamma, gae_lambda=gae_lambda, num_cpu=num_cpu, env=e_env, 
            horizon=horizon)
        stats = agent.train_step(**args)
        train_curve[i] = stats[0]
        ...
        if evaluation_rollouts is not None and evaluation_rollouts > 0:
            print("Performing evaluation rollouts ........")
            eval_paths = sample_paths(num_traj=evaluation_rollouts, policy=agent.policy, num_cpu=num_cpu,
                                      env=e_env, eval_mode=True, base_seed=seed)#e.env_id
            mean_pol_perf = np.mean([np.sum(path['rewards']) for path in eval_paths])
            if agent.save_logs:
                agent.logger.log_kv('eval_score', mean_pol_perf)
                try:
                    eval_success = e_env.env.env.evaluate_success(eval_paths)
                    agent.logger.log_kv('eval_success', eval_success)
                except:
                    pass
