...
def run_simulation(frame_callback=None):
    # Keep your existing environment and policy-loading code here.
    envw = gym.make(env_name)
    env = envw.unwrapped

    # Keep your existing policy-loading code.
    # pi = ...
...
def main(env_name, policy_path, mode, seed, num_episodes, render, camera_name, output_dir, 
         output_name, save_paths, plot_paths, render_visuals, env_args):
    
    policy_buf = open(policy_path, 'rb')
    policy = PolicyUnpickler(policy_buf).load()
    
    #A pi = pickle.load(policy)
    ...
    if policy_path is not None:
        ...
        if pi is not None:
            ...
            policy_l = open(policy_path, 'rb')
            pi = PolicyUnpickler(policy_l).load()
            #pi = pickle.load(policy_l)
    ...
    frame_callback = test_frame_callback if ENABLE_WEB_TEST else None
    render = "none" if ENABLE_WEB_TEST else "onscreen"
...
if __name__ == '__main__':
    #main()
    run_simulation(frame_callback=test_frame_callback)
